...
def attribution_patch(
    ...
    corrupted_logit_diff = (
            corrupted_logits[batch_range, -1, source_answer_index.unsqueeze(0)] - corrupted_logits[batch_range, -1, target_answer_index]
    )   
    ...
    with nnmodel.trace(source_tokens, attention_mask=source_attention_mask, scan=scan, validate=validate):
        ...
        clean_logit_diff = (
            clean_logits[batch_range, -1, source_answer_index.unsqueeze(0)] - clean_logits[batch_range, -1, target_answer_index]
        ).detach().save()
    ...
    with nnmodel.trace(target_tokens, attention_mask=target_attention_mask, scan=scan, validate=validate):
        ...
        logit_diff = (
            logits[batch_range, -1, source_answer_index]
            - logits[batch_range, -1, target_answer_index]
        )
