...
class LowRankOrthogonalProjection(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, source, target):
        ...
        # h_t = (I-P) h_t + P h_s
        return torch.matmul(target.to(self.weight.dtype), orthogonal_complement.T) + torch.matmul(
            source.to(self.weight.dtype), P.T
        )
    ...
    def constant_forward(self, source_constant, target):
        ...
        # h_t = (I-P) h_t + P u c(w)
        return torch.matmul(target.to(self.weight.dtype), orthogonal_complement.T) + torch.matmul(
            source_constant.to(self.weight.dtype).unsqueeze(1), self.weight.T
        ).squeeze(1)
    ...
    def load_pretrained(path):
        warnings.warn(
            "Loading pretrained projection using load_pretrained() path is deprecated. Use the .from_pretrained() method instead."
        )
...
def create_dataset(
    source_tokens, target_tokens, source_label_index, target_label_index, source_attn_mask, target_attn_mask, *args
):
    ...
    for eval_pred, eval_label in zip(eval_preds, eval_labels):
        ...
        if verbose:
            for b_idx in range(len(eval_pred)):
                print(
                    "Pred:",
                    tokenizer.decode(pred_test_labels[b_idx].item()),
                    " Labels (source):",
                    tokenizer.decode(eval_label[b_idx][0].item()),
                    " Labels (target):",
                    tokenizer.decode(eval_label[b_idx][1].item()),
                )
...
def train_projection(
    model: nn.Module,
    projection: LowRankOrthogonalProjection,
    layer,
    train_dataset,
    val_dataset,
    batch_size=32,
    ...
    try:
        ...
        raise ImportError(
            "Training a projection is currently still based on pyvene. Please install pyvene with `pip install pyvene` to use this feature."
        )
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn, num_workers=num_workers
    )
