...
class Site:
    def __init__(
        ...
        if layer is not None:
            assert layer >= 0, "Doesn't accept negative layers."

        self.component_name = site_name
        self.layer = layer

        if head is not None:
            assert isinstance(head, int), f"head should be an int, not {type(head)}"
        self.head = head
    ...
    def _attribution_args_validation(self, nnmodel, clean_site, zero):
        ...
        if (
            self._cache is None
            or (clean_site is not None and clean_site._cache is None)
            or self._gradient_cache is None
        ):
            raise ValueError(
                "Cache not found. Please run .cache() (with gradient=True on corrupted) before running attribution."
            )
    ...
    def cache(self, nnmodel, gradient=False):
        if self.component_name not in HeadSite._cache[self.cache_name][self.layer]:
            ...
            if gradient and (self.component_name not in HeadSite._gradient_cache[self.cache_name][self.layer]):
                HeadSite._gradient_cache[self.cache_name][self.layer][self.component_name] = (
                    proxy.grad.detach().cpu().save()
                )
...
class MultiSite(Site):
    ...
    def average_cache_over_samples(self, nnmodel, other_sites=[]):
        for site in other_sites:
            assert isinstance(
                site, MultiSite
            ), "Only MultiSite is supported for other_sites when averaging cache over samples."
...
class Sites:
    def __init__(
        ...
        for site in self.component_names:
            ...
            if seq_pos_type in ["each", "lastk", "custom"]:
                self.shape_names[site].append("seq_pos")
            # Add "head" and "neuron", if required
            if site in ["q", "o"]:
                self.shape_names[site].append("head_qo")
            if site in ["k", "v"]:
                self.shape_names[site].append("head_kv")
    ...
    def iter(
        self, site_name: str, shape_names: List[str], shape_values: Dict[str, int], seq_pos_indices: Optional[Tensor]
    ):
        ...
        if not len(self.range_config):
            if "head_qo" in shape_names:
                heads = range(shape_values["head_qo"])
            elif "head_kv" in shape_names:
                heads = range(shape_values["head_kv"])
            for layer in layers:
                for head in heads:
                    for pos in seq_pos:
    ...
    def get_sites_dict(
        self, model: NNsight, api: ModelAPI, tensor=None, cache_name="default"
        ...
        if tensor is not None:
            ...
            seq_len = 1
        self.shape_values_all = {
            "seq_pos": seq_len,
            "layer": api.N_LAYERS(model),
            "head_qo": api.N_QO_HEADS(model),
            "head_kv": api.N_KV_HEADS(model),
        }
